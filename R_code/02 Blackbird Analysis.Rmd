...
m1 <- lme(
  ...
  random = list(
    ...
    Rep = ~1 | Batch), # rep nested within batch to match experiment structure
  data = BB_df,
  method = "REML"
)
...
|  |  |  |
|--------------------|----------------|------------------------------------|
| Likelihood ratio w/ batch | 60 | the model with batch explains more structure in the data than the model without |
| P value m1 w/ batch | \< 0.0001 | batch contributes significant variance to disease severity |
| Delta AIC | 58 | very strong support (AIC \> 10 = strong support) |
...
```
Residual variance increases with disease severity. This happens from bin 2 (low-moderate severity) to bin 3 (moderate-high severity)
...
write_csv(emm_df, file = "8dpi/BB_emmeans.csv")
```
```{r}
VarCorr(model)
```
...
Counting the entries whose estimated mean is statistically distinguishable from a control given the residual variance structure of that method

-   Number of accessions sig. diff from G33312 (susceptible control) --\> 23 -- aka only 23 accessions are cleanly distinguishable from G33312
-   Number of accessions sig. diff from Santhica 27 (resistant control) --\> 36
...
```{r}
write_csv(merged_df_BB, "8dpi/BB_Dunnett.csv")
```
...
dunnett <- read.csv("8dpi/BB_Dunnett.csv")
...
p1 <- ggplot(df_fig1, aes(x = reorder(Accession, X8dpi_mean), y = X8dpi_mean, fill = Group)) +
  ...
  geom_errorbar(aes(ymin = X8dpi_mean - X8dpi_stderr, ymax = X8dpi_mean + X8dpi_stderr), 
                ...
                color = "#000000",
                alpha = 0.6) +
  geom_text(aes(y = sig_y, label = sig_label), color = "#7C6BAF", size = 2) +  # purple asterisk
  scale_fill_manual(
    values = fill_colors,
...
ggsave("8dpi/Figures/Blackbird_8dpi.tiff", plot = p1, dpi = 600, width = 6.5, height = 4.5, units = "in", compression = "lzw")
```
...
```{r}
VarCorr(bb_blup_model)
```

In BB blup model the variance decomposition is:
Batch = 0.046 
Rep = 0.077
Accession = ~0
Residual = ~0
